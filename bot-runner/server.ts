...
interface Step {
  ...
  url_pattern?: string;
  text_pattern?: string;
  /** Seite, auf der der Schritt ausführbar ist (URL-Glob mit * oder text=…). */
  on_page?: string;
}
...
/** Erkannte Seitenzustände des aktuellen Ablaufs. */
type PageState = "ok" | "captcha" | "maintenance" | "legitimation" | "confirm" | "login";

/**
 * Erkennt, auf welcher Art Seite sich der Browser gerade befindet –
 * Grundlage dafür, dass der Bot den Ablauf „versteht“ statt blind zu warten.
 */
async function detectPageState(page: Page): Promise<PageState> {
  const probe = await page.evaluate(() => {
    const text = (document.body?.innerText ?? "").slice(0, 4000);
    const head = (document.querySelector("h1")?.textContent ?? "").slice(0, 200);
    const hasPassword = !!document.querySelector("input[type=password]");
    const captcha = !!document.querySelector(
      "iframe[src*='recaptcha'], iframe[src*='hcaptcha'], iframe[src*='challenges.cloudflare'], iframe[src*='captcha'], .g-recaptcha, .h-captcha, #cf-challenge-running",
    );
    return { text, head, hasPassword, captcha };
  }).catch(() => null);
  if (!probe) return "ok";
  const { text, head } = probe;

  if (probe.captcha || /captcha|ich bin kein roboter|sicherheitsüberprüfung|unusual traffic|are you a robot/i.test(text)) {
    return "captcha";
  }
  if (/wartungsarbeiten|temporarily unavailable|service (?:vorübergehend )?(?:nicht verfügbar|unavailable)/i.test(text)) {
    return "maintenance";
  }
  if (/videoident|postident|legitimation|identifizierung|phototan|ausweis.{0,20}(?:prüfen|hochladen|bereithalten)/i.test(text)) {
    return "legitimation";
  }
  if (/(?:vorgangsnummer|antragsnummer|referenznummer|vorgangs-?id)\s*[:#-]?\s*[A-Z0-9][A-Z0-9./_-]{4,}/i.test(text)) {
    return "confirm";
  }
  if (probe.hasPassword && /anmelden|einloggen|log ?in|sign in/i.test(head) && !/registrieren|konto eröffnen|antrag/i.test(text)) {
    return "login";
  }
  return "ok";
}

/** Prüft, ob ein on_page-Marker (URL-Glob oder text=…) zur aktuellen Seite passt. */
function matchesOnPage(marker: string, url: string, bodyText: string): boolean {
  const m = marker.trim();
  if (m.toLowerCase().startsWith("text=")) {
    return new RegExp(escapeRe(m.slice(5).trim()), "i").test(bodyText);
  }
  return globToRe(m).test(url);
}
...
async function runSteps(page: Page, run: Run, steps: Step[]) {
  ...
  /** Übergabe an den Admin mit Screenshot und gesicherter Sitzung. */
  const handoff = async (stepIndex: number, reason: string): Promise<"handoff"> => {
    ...
    if (shot) {
      shotPath = `bot-runs/${run.id}/auto-${Date.now()}.png`;
    ...
    await db.from("bot_runs").update({
      ...
      resume_step: stepIndex,
      storage_state: storage,
      ...(shotPath ? { screenshot_path: shotPath } : {}),
    }).eq("id", run.id);
    await appendLog(run.id, log, `Übergabe an Admin: ${reason}`);
    return "handoff";
  };

  /** Liest die Vorgangsnummer von einer Bestätigungsseite (auch ohne extract-Schritt). */
  const tryAutoExtract = async (): Promise<boolean> => {
    if (vars.vorgangsnummer) return true;
    const source = await page.locator("body").innerText({ timeout: 5000 }).catch(() => "");
    const match = source.match(/(?:Vorgangsnummer|Antragsnummer|Referenznummer|Vorgangs-?ID|TID)\s*[:#-]?\s*([A-Z0-9][A-Z0-9./_-]{4,})/i);
    ...
    log = await appendLog(run.id, log, `Vorgangsnummer automatisch erkannt: ${caseNumber}`);
    return true;
  };
  ...
  for (let i = startAt; i < steps.length; i++) {
    ...
    // Ablauf-Erkennung: auf welcher Seite stehen wir gerade?
    if (["fill", "click", "select", "wait", "wait_for"].includes(step.action)) {
      const state = await detectPageState(page);
      if (state === "captcha") {
        return handoff(i, "Captcha/Sicherheitsprüfung erkannt – bitte manuell lösen und den Lauf danach fortsetzen.");
      }
      if (state === "legitimation") {
        return handoff(i, "Legitimationsseite erkannt (VideoIdent/PostIdent/TAN) – bitte manuell durchführen.");
      }
      if (state === "maintenance") {
        throw new PageUnavailableError(`Die Seite ${page.url()} meldet Wartungsarbeiten bzw. ist nicht verfügbar.`);
      }
      if (state === "login") {
        return handoff(i, "Login-Seite statt Antragsstrecke erkannt – Zugang erwartet. Bitte prüfen und den Lauf fortsetzen.");
      }
      if (state === "confirm") await tryAutoExtract();

      // Schritt-Synchronisation: passt die Seite zum on_page-Marker des Schritts?
      if (step.on_page) {
        const url = page.url();
        const bodyText = await page.locator("body").innerText({ timeout: 5000 }).catch(() => "");
        if (!matchesOnPage(step.on_page, url, bodyText)) {
          let jump = -1;
          for (let j = i + 1; j < steps.length; j++) {
            const candidate = steps[j];
            if (candidate?.on_page && matchesOnPage(candidate.on_page, url, bodyText)) { jump = j; break; }
          }
          if (jump > i) {
            const target = steps[jump]!;
            log = await appendLog(run.id, log,
              `Seite passt nicht zu Schritt ${i + 1} – synchronisiere auf Schritt ${jump + 1} (${target.label ?? target.action})`);
            i = jump - 1;
            continue;
          }
          // Screenshot mitschreiben, damit im Log sofort sichtbar ist,
          // auf welcher Seite der Bot tatsächlich steht.
          const diag = await captureDiagnostics(page, run.id, `mismatch-step-${i + 1}`).catch(() => null);
          log = await appendLog(run.id, log,
            `Hinweis: Seite passt nicht zum Marker von Schritt ${i + 1} (${step.on_page}) – aktuelle URL: ${url}` +
            (diag?.screenshot_path ? ` – Screenshot: ${diag.screenshot_path}` : "") + " – versuche trotzdem");
        }
      }
    }
    ...
    try {
      switch (step.action) {
        ...
        case "fill": {
          ...
          // Checkbox/Radio: echtes Anhaken statt Texteingabe
          const inputType = ((await el.getAttribute("type").catch(() => null)) ?? "").toLowerCase();
          if (inputType === "checkbox" || inputType === "radio") {
            await el.check({ timeout }).catch(async () => { await el.click({ timeout }); });
            break;
          }

          let v = value;
          const placeholder = ((await el.getAttribute("placeholder").catch(() => null)) ?? "");
          // Datumsformat der Seite beachten (TT.MM.JJJJ ↔ JJJJ-MM-TT)
          if (inputType === "date" && /^\d{2}\.\d{2}\.\d{4}$/.test(v)) {
            const [d, m, y] = v.split(".");
            v = `${y}-${m}-${d}`;
          } else if (inputType !== "date" && /^\d{4}-\d{2}-\d{2}$/.test(v) && /(tt|dd)[./](mm)/i.test(placeholder)) {
            const [y, m, d] = v.split("-");
            v = placeholder.includes("/") ? `${d}/${m}/${y}` : `${d}.${m}.${y}`;
          }
          // IBAN o. Ä.: ist das Feld kürzer als der Wert, Leerzeichen entfernen
          const maxLen = Number(await el.getAttribute("maxlength").catch(() => null));
          if (maxLen && v.length > maxLen && v.replace(/\s/g, "").length <= maxLen) {
            v = v.replace(/\s/g, "");
          }
          await el.fill(v, { timeout });
          break;
        }
        case "click":
          await assertCurrentPageOk(page);
        ...
        case "advance": {
          ...
          const NEXT_NAMES = /^(Weiter|Fortfahren|Bestätigen|Antrag absenden|Konto eröffnen|Jetzt eröffnen|Jetzt starten|Zur Kontoeröffnung|Antrag weiterführen|Weiter zum Antrag|Speichern und weiter)$/i;
          let lastSignature = "";
          let stagnant = 0;
          for (let clickIndex = 0; clickIndex < maxClicks; clickIndex++) {
            // Seitenzustand prüfen: Bestätigung, Legitimation oder Captcha erreicht?
            const state = await detectPageState(page);
            if (state === "captcha") {
              return handoff(i, "Captcha/Sicherheitsprüfung während des Weiterklickens erkannt – bitte manuell lösen.");
            }
            if (state === "legitimation") {
              log = await appendLog(run.id, log, "Legitimationsseite erreicht – Weiterklicken beendet");
              break;
            }
            if (state === "confirm") {
              await tryAutoExtract();
              log = await appendLog(run.id, log, "Bestätigungsseite erreicht – Weiterklicken beendet");
              break;
            }

            // Stagnation: Seite ändert sich nach Klicks nicht mehr → aufhören
            const bodyText = await page.locator("body").innerText({ timeout });
            const signature = `${page.url()}|${bodyText.slice(0, 300)}`;
            if (signature === lastSignature) {
              stagnant++;
              if (stagnant >= 2) {
                log = await appendLog(run.id, log, "Seite ändert sich nach Klicks nicht mehr – Weiterklicken beendet");
                break;
              }
            } else {
              stagnant = 0;
            ...
            const next = page.getByRole("button", { name: NEXT_NAMES })
              .or(page.getByRole("link", { name: NEXT_NAMES })).first();
