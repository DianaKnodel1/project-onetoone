...
//
// Stufen:
//  1) beworben            applications (is_test=false, alle flow_types inkl. classic)
...
export const getCohortStats = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    // 1) Bewerbungen – seitenweise laden, sonst schneidet die API bei 1000 Zeilen ab
    //    (das verfälschte bei 90/180 Tagen alle Zahlen nach unten).
    const PAGE = 1000;
    const allApps: any[] = [];
    for (let from = 0; ; from += PAGE) {
      ...
      const { data: page, error: appErr } = await appQ;
    ...
    // 2) Profile (registriert + onboarded) – in Blöcken, damit die Anfrage-URL
    //    bei vielen Bewerbungen nicht zu lang wird.
    type ProfRow = { email: string; onboarding: string | null };
    const profByEmail = new Map<string, ProfRow>();
    const EMAIL_CHUNK = 200;
