...
export interface CleanStep {
  ...
  url_pattern?: string;
  var_name?: string;
  /** Seite, auf der der Schritt ausführbar ist (URL-Muster mit * oder text=…). */
  on_page?: string;
}
...
/** Klick-Beschriftungen, die den Antrag abschließen. */
const FINAL_SUBMIT_RE =
  /(absenden|konto eröffnen|jetzt eröffnen|antrag (?:abschließen|einreichen|absenden)|verbindlich (?:bestellen|bestätigen))/i;
 ...
 * eingegebene Werte durch Platzhalter ersetzen, Ladezeiten als Warteschritte
 * ergänzen und jede Aktion mit einem on_page-Marker versehen, damit der
 * Runner erkennt, auf welcher Seite er sich befindet.
 */
export function cleanRecording(raw: RawRecordedStep[], startUrl?: string): CleanResult {
  ...
  let removed = 0;
  let waits = 0;
  let sawFinalSubmit = false;
  /** on_page-Marker aus der URL, auf der die Aktion aufgezeichnet wurde. */
  const onPage = (u?: string) => (u ? urlPattern(u) : undefined);
  ...
  for (let i = 0; i < events.length; i++) {
    ...
    // Längere Pause nach Klick/Auswahl, aber ohne Seitenwechsel: die Seite lädt
    // per JavaScript nach (SPA). Wartezeit übernehmen, damit der Runner später
    // nicht in Timeouts läuft. Pausen nach Tippen sind Denkzeit und zählen nicht.
    if (
      prev && ["click", "submit", "check", "select"].includes(prev.kind) &&
      e.t - prev.t > 3000
    ) {
      const last = steps[steps.length - 1];
      if (last && last.action !== "wait" && last.action !== "wait_for") {
        steps.push({ action: "wait", value: String(Math.min(e.t - prev.t, 5000)), label: "Ladezeit abwarten" });
        waits++;
      }
    }
    ...
    if (e.kind === "submit") {
      ...
      sawFinalSubmit = true;
      steps.push({ action: "click", selector, label: e.label || "Formular absenden", on_page: onPage(e.url) });
      continue;
    }
    ...
    if (e.kind === "click") {
      ...
      if (FINAL_SUBMIT_RE.test(e.label ?? "")) sawFinalSubmit = true;
      const last = steps[steps.length - 1];
      if (last?.action === "click" && last.selector === selector && prev && e.t - prev.t < 800) {
        removed++; continue; // Doppelklick
      }
      steps.push({ action: "click", selector, label: e.label || "Klick", on_page: onPage(e.url) });
      continue;
    }

    if (e.kind === "check") {
      steps.push({ action: "click", selector, label: e.label || "Auswahl setzen", on_page: onPage(e.url) });
      continue;
    }

    if (e.kind === "select") {
      steps.push({ action: "select", selector, value: e.sample ?? "", label: e.label || "Auswahl", on_page: onPage(e.url) });
      continue;
    }

    // Eingabe: mehrfaches Tippen ins selbe Feld zusammenfassen.
    // Passwort-Felder nutzen immer den Zugangsdaten-Platzhalter.
    const key = e.type === "password" ? "password" : (guessPlaceholder(e) || `feld_${++unnamed}`);
    ...
    steps.push({ action: "fill", selector, value: `{{${key}}}`, label: e.label || key, on_page: onPage(e.url) });
  }

  // Abschluss erkannt (Absenden/Konto eröffnen): Bestätigungs-Bausteine anhängen.
  if (sawFinalSubmit) {
    steps.push({ action: "advance", value: "8", label: "Bis zur Bestätigung weiterklicken" });
    steps.push({ action: "extract", label: "Vorgangsnummer auslesen" });
  }
  ...
  if (waits) notes.push(`${waits} Ladezeit-Schritt(e) aus Pausen in der Aufnahme ergänzt.`);
  if (sawFinalSubmit) {
    notes.push("Abschluss erkannt: Weiterklicken bis zur Bestätigung und Auslesen der Vorgangsnummer angehängt.");
  }
  if (placeholders.has("password")) {
    notes.push("Passwort-Feld erkannt: nutzt {{password}} aus den Zugangsdaten – kein Klartext gespeichert.");
  }
  ...
  notes.push("Jede Aktion trägt einen on_page-Marker – der Runner erkennt damit, ob er auf der richtigen Seite ist.");
