...
// Zusätzlich überwachte Domains: Landing-Seiten und manuelle Watchlist.
// Werden einfach gepingt (kein portal.-Subdomain-Check wie bei Tenants).
export interface ExtraDomainHealth {
  domain: string;
  source: "landing_page" | "watchlist";
  label: string | null;         // Landing-Slug oder Watchlist-Notiz
  watchlist_id?: string | null; // zum Entfernen aus der Watchlist
  status: DomainStatus;
  http_status: number | null;
  latency_ms: number | null;
  error: string | null;
...
// Letzten Prüfstand für Banner/Übersicht speichern. Best-effort: schlägt still
// fehl, solange die Migration domain_check_results noch nicht eingespielt ist.
async function persistCheckResults(sb: any, rows: PersistableResult[]) {
  if (rows.length === 0) return;
  try {
    await sb.from("domain_check_results").upsert(
      rows.map((r) => ({ ...r, checked_at: new Date().toISOString() })),
      { onConflict: "domain" }
    );
  } catch (e) {
    console.warn("domain_check_results nicht beschreibbar (Migration ausstehend?)", e);
  }
}
...
export const checkDomainsHealth = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }) => {
    ...
    // Zusätzlich: Landing-Seiten-Domains + manuelle Watchlist überwachen.
    // Best-effort – fehlt die Watchlist-Tabelle noch, läuft der Rest normal weiter.
    const tenantDomains = new Set(results.map((r) => r.domain));
    const extras: ExtraDomainHealth[] = [];
    let watchlistAvailable = true;
    try {
      const [{ data: landings, error: landingError }, watchlistRes] = await Promise.all([
        sb.from("landing_pages").select("id,domain,slug,is_published"),
        sb.from("domain_watchlist").select("id,domain,note"),
      ]);
      if (landingError) console.warn("landing_pages nicht lesbar:", landingError.message);
      const watchlist = watchlistRes.error ? [] : (watchlistRes.data ?? []);
      if (watchlistRes.error) {
        watchlistAvailable = false;
        console.warn("domain_watchlist nicht lesbar (Migration ausstehend?):", watchlistRes.error.message);
      }

      const extraChecks: Promise<ExtraDomainHealth>[] = [];
      const seen = new Set<string>();
      for (const l of landings ?? []) {
        if (!l.domain) continue;
        const d = normalizeDomain(l.domain);
        if (tenantDomains.has(d) || seen.has(d)) continue;
        seen.add(d);
        extraChecks.push(
          pingDomain(d).then((r) => ({
            domain: d, source: "landing_page" as const,
            label: l.slug ? `Landing: ${l.slug}${l.is_published ? "" : " (Entwurf)"}` : "Landing-Seite",
            status: r.status, http_status: r.http_status, latency_ms: r.latency_ms,
            error: r.error, checked_url: `https://${d}/`,
          }))
        );
      }
      for (const w of watchlist) {
        ...
        extraChecks.push(
          pingDomain(d).then((r) => ({
            domain: d, source: "watchlist" as const,
            label: w.note ?? null, watchlist_id: w.id,
            status: r.status, http_status: r.http_status, latency_ms: r.latency_ms,
            error: r.error, checked_url: `https://${d}/`,
          }))
        );
      }
      extras.push(...(await Promise.all(extraChecks)));
    } catch (e) {
      console.warn("Extra-Domain-Checks fehlgeschlagen:", e);
    }

    // Prüfstand für Dashboard-Banner persistieren (best-effort)
    await persistCheckResults(sb, [
      ...results.map((r) => ({
        domain: r.domain, source: "tenant", label: r.tenant_name,
        status: r.status, http_status: r.http_status, latency_ms: r.latency_ms,
        error: r.error, checked_url: r.checked_url,
      })),
      ...extras.map((r) => ({
        domain: r.domain, source: r.source, label: r.label,
        status: r.status, http_status: r.http_status, latency_ms: r.latency_ms,
        error: r.error, checked_url: r.checked_url,
      })),
    ]);

    return { domains: results, extras, watchlist_available: watchlistAvailable, checked_at: new Date().toISOString(), pause_state };
  });

// ============================================================
// 1b) Manuelle Domain-Watchlist (beliebige Domains überwachen)
// ============================================================

export const addWatchlistDomain = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  ...
  .handler(async ({ data, context }) => {
    ...
    if (!/^[a-z0-9][a-z0-9.-]*\.[a-z]{2,}$/.test(domain)) {
      throw new Error("Ungültige Domain. Bitte nur den Hostnamen angeben, z. B. example.de");
    }
    const sb = await getSupabaseAdmin();
    const { data: row, error } = await sb
      .from("domain_watchlist")
      .upsert({ domain, note: data.note ?? null }, { onConflict: "domain" })
      .select("id,domain,note")
      .single();
    if (error) throw new Error(error.message);

    // Sofort einmal prüfen und Ergebnis persistieren
    const r = await pingDomain(domain);
    await persistCheckResults(sb, [{
      domain, source: "watchlist", label: data.note ?? null,
      status: r.status, http_status: r.http_status, latency_ms: r.latency_ms,
      error: r.error, checked_url: `https://${domain}/`,
    }]);
    return { ok: true, entry: row, check: r };
  });

export const removeWatchlistDomain = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((input: unknown) =>
  ...
  .handler(async ({ data, context }) => {
    ...
    if (error) throw new Error(error.message);
    if (row?.domain) {
      try { await sb.from("domain_check_results").delete().eq("domain", row.domain); } catch {}
    }
    return { ok: true };
  });
