...
export function BotRecorderPanel({ profiles, onSaved }: Props) {
  ...
  /** Platzhalter im (editierbaren) Ablauf-JSON + Hinweis, ob es Standardfelder sind. */
  const placeholderInfo = useMemo(() => {
    const KNOWN: Record<string, string> = {
      email: "E-Mail des Mitarbeiters",
      password: "Generiertes Passwort (Zugangsdaten)",
      first_name: "Vorname",
      last_name: "Nachname",
      ...
      verify_url: "Verifizierungslink (Rückfrage an Admin)",
      vorgangsnummer: "Wird vom Bot erkannt",
    };
    const found = new Set<string>();
    const re = /\{\{\s*([a-z0-9_]+)\s*\}\}/gi;
    let m: RegExpExecArray | null;
    while ((m = re.exec(json))) found.add(m[1]!.toLowerCase());
    return [...found].map((key) => ({ key, known: KNOWN[key] ?? null }));
  }, [json]);
        ...
        {token && (
          <div className="rounded-lg border border-status-info/40 bg-status-info/5 p-3 space-y-2 text-xs">
            ...
            <ol start={2} className="list-decimal ml-4 space-y-1 text-muted-foreground">
              ...
              <li>
                Tab wechseln, minimieren oder warten ist kein Problem. <strong>Aber nach jedem Seitenwechsel</strong>{" "}
                (neue Seite, Login-Weiterleitung, neuer Tab) prüfen, ob die Leiste unten rechts noch da ist – wenn nicht,
                das Lesezeichen einfach erneut anklicken. Die bisherigen Schritte bleiben erhalten und werden fortgesetzt.
              </li>
              <li>Auf „Stopp" klicken und hier unten „Ablauf erzeugen" wählen. Steht in der Leiste „Senden blockiert", stattdessen „Kopieren" und hier einfügen.</li>
              <li>In der Vorschau die Platzhalter prüfen: rot markierte Felder fehlen im Mitarbeiter-Profil.</li>
            </ol>
      ...
      {preview && (
        <div className="rounded-xl border bg-card p-4 space-y-3">
          ...
          {placeholderInfo.length > 0 && (
            <div className="space-y-1.5">
              <Label className="text-xs">Platzhalter im Ablauf</Label>
              <div className="flex flex-wrap gap-1.5">
                {placeholderInfo.map((p) => (
                  <Badge
                    key={p.key}
                    variant={p.known ? "secondary" : "outline"}
                    className={p.known ? "text-[10px]" : "text-[10px] border-destructive/60 text-destructive"}
                    title={p.known ?? "Kein Standardfeld – muss als Feld im Mitarbeiter-Profil gepflegt werden"}
                  >
                    {`{{${p.key}}}`}
                  </Badge>
                ))}
              </div>
              {placeholderInfo.some((p) => !p.known) && (
                <div className="rounded-md border border-destructive/40 bg-destructive/5 p-2 text-[11px] text-destructive space-y-1">
                  <p className="font-medium">
                    Achtung: {placeholderInfo.filter((p) => !p.known).length} Platzhalter ohne passendes Profilfeld.
                  </p>
                  <p className="text-destructive/90">
                    Diese Felder bleiben im Antrag leer und der Lauf bricht wahrscheinlich ab. Entweder im JSON durch einen
                    festen Wert bzw. ein Standardfeld ersetzen oder das Feld im Mitarbeiter-Profil anlegen – dann erneut prüfen.
                  </p>
                </div>
              )}
            </div>
