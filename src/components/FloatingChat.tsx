    ...
    const channel = supabase
      ...
      .subscribe((status) => {
        ...
        if (status === "CHANNEL_ERROR" || status === "TIMED_OUT" || status === "CLOSED") {
          console.warn(`[Chat Realtime] Floating-Chat-Verbindung: ${status}`);
          void syncLatest();
        }
      });
        ...
        <div className="fixed bottom-6 right-6 z-50 w-[400px] max-w-[calc(100vw-2rem)] h-[580px] max-h-[calc(100vh-4rem)] bg-card border border-border rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-slide-up">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            ...
            {humanMessages.map((msg) => {
              ...
              // Automatische Nachrichten (Willkommen, Vertrag, Auftrag, SMS-Code …)
              // zentriert als Systemnachricht – nicht als Bubble des Teamleiters.
              if (!isMine && isSystemMessage(msg, teamLeaderId ?? fallbackPartnerId)) {
                return (
                  <div key={msg.id} className="flex justify-center my-1">
                    <div className="bg-muted/60 border border-border rounded-2xl px-4 py-2.5 max-w-[85%]">
                      <p className="text-[12px] text-foreground/80 whitespace-pre-wrap leading-relaxed">{msg.message}</p>
                      <p className="text-[9px] text-muted-foreground/60 mt-1 text-center">{formatTime(msg.created_at)}</p>
                    </div>
                  </div>
                );
              }
