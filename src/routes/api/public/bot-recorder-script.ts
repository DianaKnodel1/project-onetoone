...
const SCRIPT = String.raw`(function () {
  ...
  var resumed = all.length > 0;
  function paint() {
    var status = uploadFail > 0 && uploadOk === 0 ? " · Senden blockiert – „Kopieren“ nutzen" : "";
    var prefix = resumed ? "Aufnahme fortgesetzt · " : "Aufnahme läuft · ";
    text.textContent = prefix + all.length + " Schritte" + status;
  }
  ...
  if (resumed) {
    ...
    hint.textContent = "Nach jedem Seitenwechsel Lesezeichen erneut anklicken.";
    hint.style.cssText = "position:fixed;z-index:2147483646;right:16px;bottom:64px;background:#f59e0b;color:#111827;" +
      "font:12px/1.3 system-ui,sans-serif;padding:6px 10px;border-radius:8px;max-width:260px";
  ...
  window.addEventListener("beforeunload", function () {
    flush(false);
    // Nach einem echten Seitenwechsel ist dieses Skript weg – der Merker sorgt
    // dafür, dass die nächste Injektion als Fortsetzung erkennbar ist.
    try { sessionStorage.setItem("__botRecActive", "1"); } catch (e) {}
  });

  function stop() {
    try { sessionStorage.removeItem("__botRecActive"); } catch (e) {}
