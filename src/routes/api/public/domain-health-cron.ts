...
// Wird vom pg_cron alle 5 Min angefragt. Pingt alle überwachten Domains:
// Tenant-Domains (primary + aliases), Landing-Seiten-Domains und die manuelle
// Watchlist. Der letzte Prüfstand landet in `domain_check_results` (Basis für
// das Warn-Banner im Admin-Dashboard). Ins Activity-Log wird nur noch bei
// Status-Wechsel auf `down` geschrieben (kein Log-Spam alle 5 Min).
//
  ...
  server: {
    handlers: {
      GET: async ({ request }) => {
        ...
        // Vorherige Prüfstände laden, um Status-Wechsel zu erkennen
        // (Activity-Log nur bei NEUEM Ausfall, nicht alle 5 Min).
        const prevStatus = new Map<string, string>();
        try {
          const { data: prevRows } = await sb.from("domain_check_results").select("domain,status");
          for (const p of prevRows ?? []) prevStatus.set(p.domain, p.status);
        } catch {} // Tabelle evtl. noch nicht migriert – dann ohne Wechsel-Erkennung

        const results: any[] = [];
        const extras: any[] = [];
        const warned: string[] = [];
        const persist: any[] = [];
        const tenantDomainSet = new Set<string>();
        ...
        for (const t of tenants ?? []) {
          ...
          let prevDownCount = 0;
          for (const d of all) {
            tenantDomainSet.add(d);
            const r = await checkDomain(d);
            results.push({ tenant_id: t.id, tenant_name: t.name, domain: d, is_primary: d === primary, ...r });
            persist.push({
              domain: d, source: "tenant", label: t.name,
              status: r.status, http_status: r.http_status, latency_ms: r.latency_ms,
              error: r.error, checked_url: r.checked_url, checked_at: new Date().toISOString(),
            });

            if (prevStatus.get(d) === "down") prevDownCount++;
            // Activity-Log nur beim Wechsel AUF down (neuer Ausfall)
            if (r.status === "down") {
              downCount++;
              if (prevStatus.has(d) && prevStatus.get(d) !== "down") {
                try {
          ...
          // Erreichbarkeit der Landing-Domain. Warnung nur bei NEUEM Komplettausfall.
          if (all.length > 0 && downCount === all.length && prevDownCount < all.length) {
            try {
              await sb.from("activity_log").insert({
        ...
        // Zusätzlich: Landing-Seiten-Domains + manuelle Watchlist (einfacher Ping)
        try {
          const [{ data: landings }, { data: watchlist }] = await Promise.all([
            sb.from("landing_pages").select("id,domain,slug,is_published"),
            sb.from("domain_watchlist").select("id,domain,note"),
          ]);
          const seen = new Set<string>();
          const extraSources: { domain: string; source: string; label: string | null }[] = [];
          for (const l of landings ?? []) {
            if (!l.domain) continue;
            const d = normalizeDomain(l.domain);
            if (tenantDomainSet.has(d) || seen.has(d)) continue;
            seen.add(d);
            extraSources.push({ domain: d, source: "landing_page", label: l.slug ? `Landing: ${l.slug}` : "Landing-Seite" });
          }
          for (const w of watchlist ?? []) {
            const d = normalizeDomain(w.domain);
            if (tenantDomainSet.has(d) || seen.has(d)) continue;
            seen.add(d);
            extraSources.push({ domain: d, source: "watchlist", label: w.note ?? null });
          }
          for (const e of extraSources) {
            const r = await pingDomain(e.domain);
            extras.push({ ...e, ...r, checked_url: `https://${e.domain}/` });
            persist.push({
              domain: e.domain, source: e.source, label: e.label,
              status: r.status, http_status: r.http_status, latency_ms: r.latency_ms,
              error: r.error, checked_url: `https://${e.domain}/`, checked_at: new Date().toISOString(),
            });
            if (r.status === "down" && prevStatus.has(e.domain) && prevStatus.get(e.domain) !== "down") {
              try {
                await sb.from("activity_log").insert({
                  action: "domain_down_alert",
                  entity_type: "domain",
                  comment: `Überwachte Domain ${e.domain} (${e.source === "watchlist" ? "Watchlist" : "Landing-Seite"}${e.label ? `, ${e.label}` : ""}) ist DOWN (${r.error ?? "keine Antwort"}).`,
                });
              } catch {}
            }
          }
        ...
        // Prüfstände persistieren (Basis für Dashboard-Banner). Best-effort.
        try {
          await sb.from("domain_check_results").upsert(persist, { onConflict: "domain" });
        } catch (e) {
          console.warn("domain_check_results nicht beschreibbar (Migration ausstehend?)", e);
        }

        return Response.json({ ok: true, checked_at: new Date().toISOString(), domains: results, extras, warned_all_offline: warned, auto_paused: [] });
      },
    },
  },
});
