  ...
  server: {
    handlers: {
      ...
      POST: async ({ request }) => {
        ...
        let base = (parsed.data.portal_url || new URL(request.url).origin).replace(/\/+$/, "");
        // Hinter Cloudflare kommt der Origin intern oft als http:// an —
        // Magic-Links muessen immer https sein (Token nicht unverschluesselt).
