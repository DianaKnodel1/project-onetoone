...
function AdminAssignmentDetailPage() {
  ...
  const updateStatus = async (status: AssignmentStatus) => {
    ...
    // Vergütung: bei Ablehnung ausstehende Transaktionen entfernen (kein
    // Phantom-Guthaben), bei Genehmigung die vorhandene ausstehende
    // Transaktion umstellen statt eine zweite anzulegen (keine Doppelzählung).
    if (status === "abgelehnt") {
      await supabase.from("user_transactions").delete()
        .eq("assignment_id", assignment.id).eq("status", "ausstehend");
    }
    if (status === "genehmigt" && template && template.compensation > 0) {
      const { data: pendingTx } = await supabase.from("user_transactions")
        .select("id").eq("assignment_id", assignment.id).eq("status", "ausstehend").limit(1);
      if (pendingTx && pendingTx.length > 0) {
        await supabase.from("user_transactions")
          .update({ status: "genehmigt" } as any).eq("id", pendingTx[0].id);
      } else {
    ...
    // Mitarbeiter benachrichtigen (Glocke im Portal)
    if (status === "abgelehnt" || status === "nachbesserung" || status === "genehmigt") {
      const tplTitle = template?.title ?? "Auftrag";
      const notifMap: Record<string, { title: string; message: string }> = {
        abgelehnt: {
          title: "Auftrag abgelehnt",
          message: `„${tplTitle}" wurde leider abgelehnt.${comment ? ` Grund: ${comment}` : ""} Du kannst den Auftrag korrigieren und erneut einreichen.`,
        },
        nachbesserung: {
          title: "Nachbesserung angefordert",
          message: `„${tplTitle}" braucht eine Nachbesserung.${comment ? ` Hinweis: ${comment}` : ""} Schau dir das Feedback an und reiche erneut ein.`,
        },
        genehmigt: {
          title: "Auftrag genehmigt",
          message: `„${tplTitle}" wurde genehmigt.${template && template.compensation > 0 ? ` Vergütung: ${Number(template.compensation).toFixed(2).replace(".", ",")} €.` : ""}`,
        },
      };
      const n = notifMap[status];
      if (n) {
        await supabase.from("notifications").insert({
          user_id: assignment.user_id, type: "assignment_status",
